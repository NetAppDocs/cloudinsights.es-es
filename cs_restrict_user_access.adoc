...
[role="lead"]
Una vez detectado un ataque, Cloud Secure puede detener el ataque bloqueando el acceso del usuario al sistema de archivos. El acceso se puede bloquear automáticamente, mediante Directivas de respuesta automática o manualmente desde las páginas de alerta o de detalles del usuario.
...
El usuario está bloqueado directamente para las direcciones SMB e IP de los equipos host que provocan el ataque se bloqueará para NFS. Esas direcciones IP de la máquina no podrán acceder a ninguna de las máquinas virtuales de almacenamiento (SVM) supervisadas por Cloud Secure.

Por ejemplo, digamos que Cloud Secure gestiona 10 SVM y la política de respuesta automática está configurada para cuatro de dichas SVM. Si el ataque se origina en una de las cuatro SVM, el acceso del usuario se bloqueará en las 10 SVM. Se sigue utilizando una snapshot en la SVM de origen.
...
Si está utilizando un usuario personalizado (por ejemplo, _csuser_) con permisos proporcionados al usuario, siga los pasos que se indican a continuación para otorgar permisos a Cloud Secure para bloquear al usuario.
...
* En Cloud Secure, vaya a *Administración > Directivas de respuesta automatizadas > Configuración de la directiva de respuesta > Bloquear acceso de usuario*.
...
....
. Elimine las reglas en todas las directivas de la SVM que tengan “cloudsecure_rule” como Client Match especificando su respectivo RuleIndex. La regla de la Cloud Secure estará generalmente en 1.
+
 contrail-qa-fas8020::*> export-policy rule delete -vserver <svm name> -policyname * -ruleindex 1
. Asegúrese de que se ha eliminado la regla Cloud Secure (paso opcional para confirmar).
+
....
contrail-qa-fas8020::*> export-policy rule show -vserver <svm name>
             Policy          Rule    Access   Client                RO
...
Siga estos pasos para restaurar manualmente cualquier usuario de ONTAP si finaliza la prueba de Cloud Secure o si el agente/recopilador está inactivo.

Puede obtener la lista de usuarios bloqueados en Cloud Secure desde la página de lista de usuarios.
...
. Ejecute el siguiente comando para enumerar todos los usuarios bloqueados por Cloud Secure para SMB en todas las SVM:
+
