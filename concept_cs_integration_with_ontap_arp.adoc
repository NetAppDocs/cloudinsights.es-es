...
summary: Cloud Secure se integra con la protección de ransomware autónoma de ONTAP para mejorar la detección de ransomware. 
...
Cloud Secure se integra con ONTAP para recibir eventos ARP y proporciona una capa de análisis adicional y respuestas automáticas.

Cloud Secure recibe los eventos ARP de ONTAP y realiza las siguientes acciones:
...
. Volúmenes con ARP habilitado. Se pueden encontrar detalles sobre la habilitación de ARP link:https://docs.netapp.com/us-en/ontap/anti-ransomware/enable-task.html["aquí"]. ARP debe habilitarse mediante System Manager de OnCommand. Cloud Secure no puede habilitar ARP.
. Se debe añadir el recopilador Cloud Secure a través de la IP del clúster.
...
Si utiliza un usuario personalizado (por ejemplo, _csuser_) con permisos proporcionados al usuario, siga los pasos que se indican a continuación para otorgar permisos a Cloud Secure para recopilar información relacionada con ARP de ONTAP.
...
En caso de que Cloud Secure no supervise una SVM, pero existen eventos ARP generados por ONTAP, Cloud Secure seguirá recibiendo y mostrando los eventos. Sin embargo, la información forense relacionada con la alerta, así como la asignación de usuarios, no se capturará ni se mostrará.
