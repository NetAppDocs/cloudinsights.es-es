...
Cloud Insights tiene la capacidad de recopilar datos de switch de cluster ONTAP como una opción en los recopiladores <<advanced-configuration,Configuración avanzada>> configuración. Además de habilitar esto en el recopilador Cloud Insights, también debe *configurar el propio sistema ONTAP* para que lo proporcione link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["información del interruptor"], y asegúrese de que es correcto <<a-note-about-permissions,permisos>> Se establecen para permitir que los datos del conmutador se envíen a Cloud Insights.
...
| Métricas de Cluster Switch | Permita que Cloud Insights recopile datos de switches de clúster. Tenga en cuenta que además de habilitar esto en el lado de Cloud Insights, también debe configurar el sistema ONTAP para proporcionar link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["información del interruptor"], y asegúrese de que es correcto <<a-note-about-permissions,permisos>> Se establecen para permitir que los datos del conmutador se envíen a Cloud Insights. Consulte la sección “Una nota sobre los permisos” a continuación. 
|===
...
NOTE: Si el control de acceso ONTAP no está configurado correctamente, las llamadas REST DE Cloud Insights pueden fallar, lo que produce lagunas en los datos del dispositivo.  Por ejemplo, si lo ha habilitado en el recopilador Cloud Insights pero no ha configurado los permisos en el ONTAP, la adquisición fallará.  Además, si el rol se definió previamente en la ONTAP y va a añadir las habilidades de la API de REST, asegúrese de que se haya agregado _http_ al rol.
