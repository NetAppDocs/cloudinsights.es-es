...
[role="lead"]
Este recopilador de datos adquiere datos de inventario y rendimiento de sistemas de almacenamiento que ejecutan ONTAP utilizando llamadas de API de REST de solo lectura desde una cuenta de ONTAP. Este recopilador de datos también crea un registro en el registro de la aplicación de clúster para acelerar la compatibilidad.
...
* Debe tener acceso a una cuenta de administrador configurada para llamadas de API de REST DE solo lectura.
...
| Dirección IP de administración de ONTAP | La dirección IP o el nombre de dominio completo del clúster de NetApp 
...
| Recopilación avanzada de datos de contador | Seleccione esta opción para incluir datos de contador avanzado de ONTAP en sondeos. 


| Habilite la recogida de eventos de EMS | Seleccione esto para incluir los datos del evento de registro de ONTAP EMS. 
...
También debe asegurarse de que el permiso de escritura para la API de REST DE ONTAP esté habilitado. Normalmente, esto requiere una cuenta en el nivel del clúster con los permisos necesarios.
...
inicio de sesión de seguridad rest-role create -role {role name} -api /api -access readonly
inicio de sesión de seguridad rest-role create -role {role name} -api /api/cluster/agents -access all

servicios de vserver acceso web create -name spi -role {role name} -vserver {name from above command}
inicio de sesión de seguridad create -user-or-group-name {username} -application http -authentication-method password -role {role name}
