...
=== Iconos de objeto nuevo

Cloud Insights ahora tiene una iconografía de objetos actualizada, lo que reduce la varianza general de los iconos utilizados en las diversas páginas de Cloud Insights, así como la agrupación de objetos similares (por ejemplo, objetos relacionados con el volumen). Los iconos anteriores basados en letras se sustituyen por gráficos basados en imágenes, lo que hace que las pantallas Cloud Insights sean más fáciles de usar y accesibles.

¿No está seguro de lo que significa un icono? La siguiente tabla (también disponible en la link:reference_object_icons.html["Referencia de iconos de objeto"] página) las explica todas.
