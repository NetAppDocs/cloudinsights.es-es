...
=== Compatibilidad con SAN iSCSI para mapa de red de Kubernetes

Identifica las dependencias de almacenamiento subyacente de Kubernetes para NFS y ahora iSCSI.
Acelera la resolución de problemas obteniendo información sobre la actividad de red de almacenamiento de la aplicación de Kubernetes y métricas de almacenamiento sin definir. Comprende cómo la aplicación de Kubernetes utiliza la infraestructura de almacenamiento para los informes de pago por uso y visibilidad de uso.
...
Además de su prueba inicial de Cloud Insights, también puede aprovechar link:concept_subscribing_to_cloud_insights.html#module-evaluation["Evaluación del módulo"]. Por ejemplo, si ya está suscrito a Infrastructure Observability pero está añadiendo Kubernetes a su entorno, entrará automáticamente en una prueba de 30 días de Kubernetes Observability. Solo se te cobrará por el uso de la unidad gestionada por Kubernetes Observability al final del período de prueba.
...
Garantizar la accesibilidad a sus datos es fundamental, por lo que hemos realizado la exportación de .CSV image:csv_export_icon.png["icono de exportación .csv"] disponible para todas las consultas métricas, widgets de tabla de panel y páginas de destino de objetos, independientemente del tipo de datos (activo o integración) que esté consultando.
...
La página *Admin > Subscription* link:concept_subscribing_to_cloud_insights.html["tabla de comparación"] Se ha actualizado para enumerar los conjuntos de funciones disponibles en las ediciones Basic, Standard y Premium de Cloud Insights. NetApp mejora constantemente sus servicios cloud, por lo que consulte esta página a menudo para encontrar la edición adecuada para las necesidades cambiantes de su negocio.
