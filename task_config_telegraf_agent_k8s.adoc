...
toc::[]
El operador de Kubernetes y los recopiladores de datos se descargan del registro de Docker de Cloud Insights. Una vez instalado, el Operador administra los recopiladores compatibles con el Operador desplegados en los nodos del clúster de Kubernetes para adquirir datos, incluida la gestión del ciclo de vida de esos recopiladores. Después de esta cadena, los datos se adquieren de los colectores y se envían a Cloud Insights.
...
IMPORTANT: Lea la link:pre-requisites_for_k8s_operator.html["*Antes de instalar o actualizar*"] Antes de instalar o actualizar el operador de supervisión de Kubernetes de NetApp, se requiere documentación previa.
