...
Edite el despliegue de operador de supervisión en operator-config.yaml y modifique todas las referencias de imagen para utilizar su repositorio Docker privado.
...
| Después de instalar el operador, los pods de telegraf-ds ingresan CrashLoopBackOff y los registros de pod indican “su: Error de autenticación”. | Edite el agentconfiguration de supervisión-configuración y establezca _dockerMetricCollectionEnabled_ en FALSE.  Para obtener más información, consulte el apartado del operador link:telegraf_agent_k8s_config_options.html["opciones de configuración"]. 
|===
