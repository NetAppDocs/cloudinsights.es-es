...
Tenga en cuenta que el nivel de usuario predeterminado de FOS no es suficientemente potente para que Cloud Insights vea todos los aspectos lógicos de un dispositivo. Necesitamos una cuenta de usuario con la función de chasis activada, así como permisos en todas las estructuras virtuales configuradas en un conmutador.

A continuación se muestra un ejemplo de cómo puede crear una cuenta de usuario de “mínimo privilegio” para el uso de Cloud Insights en una sesión SSH en un dispositivo FOS:
...
* Debe haber conectividad TCP para todos los switches de la estructura. Este tipo de recopilador de datos probará sin problemas tanto HTTP como HTTPS de cada dispositivo de la estructura. Si activa la casilla de comprobación _Discover all switches in the fabric_, Cloud Insights identifica todos los switches de la estructura; sin embargo, necesita conectividad TCP con estos switches adicionales para detectarlos.
