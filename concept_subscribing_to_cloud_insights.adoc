...
La observabilidad de la infraestructura y la observabilidad de Kubernetes se miden por cada *unidad gestionada*. El uso de las unidades administradas se calcula en función del número de *hosts o máquinas virtuales* y de la cantidad de *capacidad sin formato* que se administra en el entorno de la infraestructura.
