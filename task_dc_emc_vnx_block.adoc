...
| Error: No se pueden recuperar los meta LUN. Error al ejecutar java -jar navicli.jar | • Modificar la configuración del recopilador de datos para utilizar el cliente seguro (recomendado)
• Instale navicli.jar en la ruta de la CLI a navicli.exe O naviseccli.exe
...
| Error: Error de revisión no coincidente | • Esto suele deberse a la actualización del firmware en el dispositivo de almacenamiento de bloques VNX, pero no a la actualización de la instalación de NaviCLI.exe. Esto también puede deberse a que haya dispositivos diferentes con distintas firmwares, pero sólo una CLI instalada (con una versión de firmware diferente).
• Verifique que el dispositivo y el host estén ejecutando versiones idénticas del software:
    - Desde la Unidad de Adquisición Cloud Insights, abra una ventana de línea de comandos
    - Cambiar el directorio al directorio de instalación configurado
    - Hacer una conexión con el dispositivo CLARiiON escribiendo “navicli -h <ip> getagent”
    - Busque el número de versión en el primer par de líneas. Ejemplo: “Agent Rev: 6.16.2 (0,1)”
